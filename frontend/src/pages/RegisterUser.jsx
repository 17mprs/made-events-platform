...
export default function RegisterUser() {
  ...
  const [duplicate,       setDuplicate]       = useState(null)  // 'LEAD_ESISTENTE' | 'TALENT_ESISTENTE'
  ...
  const submit = async (e) => {
    ...
    if (res.data?.message === 'LEAD_ESISTENTE' || res.data?.message === 'TALENT_ESISTENTE') {
      setDuplicate(res.data.message)
      return
    }
  ...
  if (duplicate) {
    const isTalent = duplicate === 'TALENT_ESISTENTE'
    return (
      <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', padding:'40px 24px', background:'#FAFAFA' }}>
        <div style={{ maxWidth:'480px', textAlign:'center' }}>
          ...
          <h1 style={{ fontSize:'20px', fontWeight:300, letterSpacing:'2px', textTransform:'uppercase', marginBottom:'12px', color:COLORS.text }}>
            {isTalent ? 'Account già attivo' : 'Già registrato'}
          </h1>
          <p style={{ fontSize:'14px', color:COLORS.textSecondary, lineHeight:1.8, marginBottom:'32px' }}>
            {isTalent ? (
              <>Hai già un account attivo su MADE EVENTS.<br />Controlla la tua casella email per il link di accesso.</>
            ) : (
              <>Sei già registrato con questa email.<br />Controlla la tua casella per il link di completamento.</>
            )}
          </p>
